...
import loguru
import argparse
from collector.parser import extract_text_from_html
...
def collect_data(driver, target_url):
    ...
    with open("src/urls.json", "w", encoding="utf-8") as f:
        json.dump(urls, f, ensure_ascii=False, indent=4)
    with open("src/html_content.html", "w", encoding="utf-8") as f:
        f.write(html_content)
    paragraphs = extract_text_from_html(html_content)    
...
def main(input_text: str):
    ...
    try:
        ...
        with open("src/result.json", "a", encoding="utf-8") as f:
            json.dump(result, f, ensure_ascii=False, indent=4)
        with open("src/paragraphs.txt", "r", encoding="utf-8") as file:
            return file.read()  
    finally:
        driver.close()
